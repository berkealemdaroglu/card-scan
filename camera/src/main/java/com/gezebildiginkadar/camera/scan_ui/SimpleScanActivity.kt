...
import com.gezebildiginkadar.camera.ui.util.asRect
import com.gezebildiginkadar.camera.ui.util.dpToPixels
import com.gezebildiginkadar.camera.ui.util.getColorByRes
import com.gezebildiginkadar.camera.ui.util.getDrawableByRes
import com.gezebildiginkadar.camera.ui.util.getFloatResource
import com.gezebildiginkadar.camera.ui.util.hide
import com.gezebildiginkadar.camera.ui.util.setDrawable
import com.gezebildiginkadar.camera.ui.util.setTextSizeByRes
import com.gezebildiginkadar.camera.ui.util.setVisible
import com.gezebildiginkadar.camera.ui.util.show
import com.gezebildiginkadar.camera.ui.ViewFinderBackground
...
abstract class SimpleScanActivity : ScanActivity() {
    ...
    protected open fun setupCloseButtonViewUi() {
        when (val view = closeButtonView) {
            is ImageView -> {
                ...
                if (isBackgroundDark()) {
                //    com.gezebildiginkadar.camera.ui.util.setDrawable(R.drawable.bouncer_close_button_dark)
                } else {
              //      com.gezebildiginkadar.camera.ui.util.setDrawable(R.drawable.bouncer_close_button_light)
                }
            }
            is TextView -> {
                view.text = getString(R.string.bouncer_close_button_description)
                if (isBackgroundDark()) {
               //     view.setTextColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerCloseButtonDarkColor))
                } else {
                //    view.setTextColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerCloseButtonLightColor))
                }
            }
        }
    }
    ...
    protected open fun setupTorchButtonViewUi() {
       // com.gezebildiginkadar.camera.ui.util.setVisible(isFlashlightSupported == true)
        when (val view = torchButtonView) {
            is ImageView -> {
                view.contentDescription = getString(R.string.bouncer_torch_button_description)
                if (isBackgroundDark()) {
                    if (isFlashlightOn) {
            ///            com.gezebildiginkadar.camera.ui.util.setDrawable(R.drawable.bouncer_flash_on_dark)
                    } else {
             //           com.gezebildiginkadar.camera.ui.util.setDrawable(R.drawable.bouncer_flash_off_dark)
                    }
                } else {
                    if (isFlashlightOn) {
             //           com.gezebildiginkadar.camera.ui.util.setDrawable(R.drawable.bouncer_flash_on_light)
                    } else {
              //          com.gezebildiginkadar.camera.ui.util.setDrawable(R.drawable.bouncer_flash_off_light)
                    }
                }
            }
            is TextView -> {
                ...
                if (isBackgroundDark()) {
              //      view.setTextColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerFlashButtonDarkColor))
                } else {
              //      view.setTextColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerFlashButtonLightColor))
                }
            }
        }
    }
    ...
    protected open fun setupSwapCameraButtonViewUi() {
        ///com.gezebildiginkadar.camera.ui.util.setVisible(hasMultipleCameras == true)
        when (val view = swapCameraButtonView) {
            is ImageView -> {
                view.contentDescription = getString(R.string.bouncer_swap_camera_button_description)
                if (isBackgroundDark()) {
            //        com.gezebildiginkadar.camera.ui.util.setDrawable(R.drawable.bouncer_camera_swap_dark)
                } else {
         //          com.gezebildiginkadar.camera.ui.util.setDrawable(R.drawable.bouncer_camera_swap_light)
                }
            }
            is TextView -> {
                view.text = getString(R.string.bouncer_swap_camera_button_description)
                if (isBackgroundDark()) {
             //       view.setTextColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerCameraSwapButtonDarkColor))
                } else {
        //            view.setTextColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerCameraSwapButtonLightColor))
                }
            }
        }
    }
    ...
    protected open fun setupViewFinderViewUI() {
       // viewFinderBorderView.background = com.gezebildiginkadar.camera.ui.util.getDrawableByRes(R.drawable.bouncer_card_border_not_found)
    }

    protected open fun setupInstructionsViewUi() {
     ///   com.gezebildiginkadar.camera.ui.util.setTextSizeByRes(R.dimen.bouncerInstructionsTextSize)
        instructionsTextView.typeface = Typeface.DEFAULT_BOLD
        instructionsTextView.gravity = Gravity.CENTER

        if (isBackgroundDark()) {
        //    instructionsTextView.setTextColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerInstructionsColorDark))
        } else {
        //    instructionsTextView.setTextColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerInstructionsColorLight))
        }
    }

    protected open fun setupSecurityNoticeUi() {
        securityTextView.text = getString(R.string.bouncer_card_scan_security)
       // com.gezebildiginkadar.camera.ui.util.setTextSizeByRes(R.dimen.bouncerSecurityTextSize)
        securityIconView.contentDescription = getString(R.string.bouncer_security_description)

        if (isBackgroundDark()) {
           // securityTextView.setTextColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerSecurityColorDark))
        //    com.gezebildiginkadar.camera.ui.util.setDrawable(R.drawable.bouncer_lock_dark)
        } else {
          //  securityTextView.setTextColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerSecurityColorLight))
         //   com.gezebildiginkadar.camera.ui.util.setDrawable(R.drawable.bouncer_lock_light)
        }
    }

    protected open fun setupCardDetailsUi() {
       // cardNumberTextView.setTextColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerCardPanColor))
       // com.gezebildiginkadar.camera.ui.util.setTextSizeByRes(R.dimen.bouncerPanTextSize)
        cardNumberTextView.gravity = Gravity.CENTER
        cardNumberTextView.typeface = Typeface.DEFAULT_BOLD


       // cardNameTextView.setTextColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerCardNameColor))
     //   com.gezebildiginkadar.camera.ui.util.setTextSizeByRes(R.dimen.bouncerNameTextSize)
    ...
    protected open fun setupDebugUi() {
      ...
      //  com.gezebildiginkadar.camera.ui.util.setVisible(Config.isDebug)
      //  com.gezebildiginkadar.camera.ui.util.setVisible(Config.isDebug)
    }
    ...
    private fun setupLogoUi() {
     ...
     //   com.gezebildiginkadar.camera.ui.util.setVisible(Config.displayLogo)
    }
    ...
    private fun setupVersionUi() {
      ...
      //  com.gezebildiginkadar.camera.ui.util.setVisible(Config.isDebug)
        ...
        if (isBackgroundDark()) {
           // versionTextView.setTextColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerSecurityColorDark))
        }
    }
    ...
    protected open fun setupViewFinderConstraints() {
        ...
        //val viewFinderMargin = (min(screenSize.width, screenSize.height) * com.gezebildiginkadar.camera.ui.util.getFloatResource(R.dimen.bouncerViewFinderMargin)).roundToInt()
        ...
        listOf(viewFinderWindowView, viewFinderBorderView).forEach { view ->
            ...
            view.addConstraints {
                /*setVerticalBias(it.id,
                                com.gezebildiginkadar.camera.ui.util.getFloatResource(R.dimen.bouncerViewFinderVerticalBias)
                )
                setHorizontalBias(it.id,
                                  com.gezebildiginkadar.camera.ui.util.getFloatResource(R.dimen.bouncerViewFinderHorizontalBias)
                )*/
    ...
    private fun setupLogoConstraints() {
        /*logoView.layoutParams = ConstraintLayout.LayoutParams(
            ///com.gezebildiginkadar.camera.ui.util.dpToPixels(LOGO_WIDTH_DP), // width
            ViewGroup.LayoutParams.WRAP_CONTENT, // height
        ).apply {
    ...
    protected open fun displayState(newState: ScanState, previousState: ScanState?) {
       /* when (newState) {
            is ScanState.NotFound -> {
                viewFinderBackgroundView.setBackgroundColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerNotFoundBackground))
            ...
            is ScanState.FoundShort -> {
                viewFinderBackgroundView.setBackgroundColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerFoundBackground))
            ...
            is ScanState.Correct -> {
                viewFinderBackgroundView.setBackgroundColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerCorrectBackground))
            ...
            is ScanState.Wrong -> {
                viewFinderBackgroundView.setBackgroundColor(com.gezebildiginkadar.camera.ui.util.getColorByRes(R.color.bouncerWrongBackground))
    ...
    override fun prepareCamera(onCameraReady: () -> Unit) {
        previewFrame.post {
          //  viewFinderBackgroundView.setViewFinderRect(com.gezebildiginkadar.camera.ui.util.asRect())
            onCameraReady()
        }
    }
    ...
    override fun onFlashSupported(supported: Boolean) {
       ...
       // com.gezebildiginkadar.camera.ui.util.setVisible(supported)
    }
    ...
    override fun onCameraStreamAvailable(cameraStream: Flow<CameraPreviewImage<Bitmap>>) {
        /*scanFlow.startFlow(
            ...
            viewFinder = com.gezebildiginkadar.camera.ui.util.asRect(),
            lifecycleOwner = this,
            coroutineScope = this,
        )*/
    }
