package com.gezebildiginkadar.camera.scan_framework
...
//        framesToProcess = frames.map { channel.trySend(it) }.count { it.isSuccess }
        ...
        framesToProcess = frames.map { channel.trySend(it).isSuccess }.count { it }
...
//    channelFlow { this@backPressureDrop.collect { trySend(it) } }.buffer(capacity = Channel.RENDEZVOUS)
    @Suppress("DEPRECATION")
    channelFlow { this@backPressureDrop.collect { this.trySend(it).isSuccess } }.buffer(capacity = Channel.RENDEZVOUS)
